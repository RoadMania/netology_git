...
provisioner "local-exec" {
  command = "echo export YC_ACCESS_KEY=${yandex_iam_service_account_static_access_key.terraform_service_account_key.access_key} > ../diploma/backend.tfvars"
}

provisioner "local-exec" {
  command = "echo export YC_SECRET_KEY=${yandex_iam_service_account_static_access_key.terraform_service_account_key.secret_key} >> ../diploma/backend.tfvars"
}
}
